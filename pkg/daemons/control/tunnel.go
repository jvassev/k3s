			...
			for _, ip := range pod.Status.PodIPs {
				if cidr, err := util.IPStringToIPNet(ip.IP); err == nil {
					if pod.DeletionTimestamp != nil {
						nets, _ := t.cidrs.ContainingNetworks(net.ParseIP(ip.IP))
						if entry, ok := nets[0].(*tunnelEntry); ok {
							if entry.podID == pod.UID {
								// only remove when the latest owner pod is deleted, otherwise a Complete Pod (job) may cause deleting the entry
